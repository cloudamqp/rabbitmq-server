...
%%
%% Everything but the bindings is gathered up front as maps (the canonical
%% definition format from rabbit_definitions, the same one used by
%% `rabbitmqctl export_definitions`), filtered to the virtual host when scoped.
%% Bindings are streamed via rabbit_definitions:fold_bindings/3 so the
%% potentially very large set is never materialised in memory.
export(Scope, ReqData0, Context) ->
    ReqData = set_download_header(ReqData0),
...
%% Gathers everything except bindings. The version/product metadata and which
%% categories are included depend on the scope; for a single virtual host the
%% per-item 'vhost' field is stripped, matching the historical behaviour.
small_parts(all) ->
    ...
    #{
        ...
        queues                     => rabbit_definitions:list_queues(),
...
small_parts({vhost, VHostName}) ->
    ...
    #{
        ...
        queues                     => Keep(rabbit_definitions:list_queues()),
        exchanges                  => Keep(rabbit_definitions:list_exchanges())
    }.

%% Buffered fallback for BERT (cannot be streamed incrementally) and HEAD
%% (which must not carry a body). Bindings are materialised here; the common
%% GET+JSON path streams them instead.
buffered(Scope, SmallParts, Encode, ReqData0, Context) ->
    ...
    ReqData = rabbit_mgmt_util:set_resp_header(<<"cache-control">>, "no-cache", ReqData0),
    {Encode(SmallParts#{bindings => Bindings}), ReqData, Context}.

%% Sets the Content-Disposition header when a ?download=<filename> query
%% parameter is present, so the export can be saved to a file by the browser.
set_download_header(ReqData) ->
    case rabbit_mgmt_util:qs_val(<<"download">>, ReqData) of
        undefined -> ReqData;
        Filename  -> rabbit_mgmt_util:set_resp_header(
                       <<"Content-Disposition">>,
                       "attachment; filename=" ++ binary_to_list(Filename), ReqData)
    end.

maybe_strip({vhost, _}, Map) -> maps:remove(<<"vhost">>, Map);
maybe_strip(all, Map)        -> Map.

%% Number of array elements (e.g. bindings) encoded per streamed chunk. Large
%% enough to keep HTTP chunk overhead low, small enough to bound peak memory.
-define(STREAM_BATCH_SIZE, 2000).

%% Streams the JSON response without ever materialising the whole document. The
%% small categories are streamed first, then bindings are folded straight from
%% the store into the socket in batches. The cowboy_rest etag/content-type
%% response headers are already set when the provider callback runs, so
%% stream_reply/3 picks them up; we only add cache-control. Returns {stop, ...}
...
stream_export(Scope, SmallParts, ReqData0, Context) ->
    ...
    %% small_parts/1 always contains metadata, so it is never empty: the
    %% bindings entry below can unconditionally be comma-prefixed.
    stream_entries(maps:to_list(SmallParts), ReqData, true),
    ok = cowboy_req:stream_body([$,, <<"\"bindings\":[">>], nofin, ReqData),
    {ReqData, First, Pending, _N} =
        rabbit_definitions:fold_bindings(
          Scope,
          fun(B, {Req, Fst, Batch, N}) ->
                  B1 = maybe_strip(Scope, B),
                  case N + 1 >= ?STREAM_BATCH_SIZE of
                      true ->
                          ok = cowboy_req:stream_body(
                                 encode_batch(lists:reverse([B1 | Batch]), Fst), nofin, Req),
                          {Req, false, [], 0};
                      false ->
                          {Req, Fst, [B1 | Batch], N + 1}
                  end
          end,
          {ReqData, true, [], 0}),
    case Pending of
...
%% Category lists (queues, exchanges, bindings, ...) are lists of maps; they
%% are streamed element by element, in batches, so a huge category never sits
%% fully encoded in memory. Any other value -- scalars, maps, and proplists
%% such as a virtual host's `limits' -- is encoded whole by rabbit_json, which
%% correctly renders proplists as JSON objects.
